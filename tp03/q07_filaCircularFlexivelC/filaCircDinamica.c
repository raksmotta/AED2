...
void removerInicio (int imprimir){
...
void inserirFim (pokemon* poke){
	...
	//se a fila está cheia, removemos
	if(tam == 5){
		removerInicio(0);
	}
    ...
    //após inserir, calcular e imprimir a media
	int soma = 0;

	pokemon* atual = primeiro->prox;

	while (atual != primeiro){
		soma += atual->captureRate;
        atual = atual->prox;
	}
