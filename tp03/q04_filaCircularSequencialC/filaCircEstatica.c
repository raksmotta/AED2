...
#define MAX_SIZE 5 //tamanho maximo da fila circular
...
//arranjo de ponteiros para pokemon
pokemon* queue[MAX_SIZE];
//frente (inicio, primeiro)
int front = 0;
//trás (fim, ultimo)
int rear = 0;
int size = 0;

void inserirFila(pokemon* p) {

    if (size == MAX_SIZE) {

        //fila cheia, precisamos remover antes de inserir
		//na fila, removemos sempre a partir do primeiro, de quem está mais na frente
        pokemon* removido = queue[front];
        //printf("(R) %s\n", removido->name);
        free_pokemon(removido);

		//movemos a frente da lista para uma posicao entre 0 e 4
        front = (front + 1) % MAX_SIZE;
        size--;
    }

	//inserimos o novo elemento no final
    queue[rear] = p;
    rear = (rear + 1) % MAX_SIZE;
    size++;

    //após inserir, calcular e imprimir a média
    int soma = 0;
    for (int i = 0; i < size; i++) {

        int index = (front + i) % MAX_SIZE;
        soma += queue[index]->captureRate;

    }

    double media = (double)soma / size;
    printf("Média: %.0lf", round(media));
	printf("\n");
}
...
int main () {
	...
	while(acabar == 0){
		...
		if (strcmp(linha, "FIM") == 0) {
			...
			if (processed_line) {
                ...
                inserirFila(parse_poke_line(processed_line));
	...
	//liberar memória alocada para os pokémons na fila
    while (size > 0) {
        pokemon* p = queue[front];
        front = (front + 1) % MAX_SIZE;
        size--;
        free_pokemon(p);
    }
