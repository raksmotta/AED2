...
#define MAX_SIZE 500 //tamanho maximo da pilha
...
// PILHA SEQUENCIAL
//um array de PONTEIROS para pokemons
pokemon* stack[MAX_SIZE];
int topo;

//inicializar topo como -1 para indicar que a pilha está vazia
void criarPilha() {
    topo = -1;
}

void inserirPilha(pokemon* poke) {

    if (topo < MAX_SIZE - 1) {

        topo++;
		//armazenamos o ponteiro para pokemon na posicao atualizada do topo
        stack[topo] = poke;
...
pokemon* removerPilha() {

    if (topo >= 0) {

        pokemon* tmp = stack[topo];
        topo--;
        return tmp; //retornamos o ponteiro para o pokémon removido
...
//simplesmente percorre o array stack incrementando as posicoes
void mostrar_pilha_normal() {

    int x = 0;

    for (int i = topo; i >= 0; i--) {
        printf("[%d] ", x);
        print_pokemon(stack[i]);
        x++;
    }
}

//função recursiva auxiliar para mostrar os elementos na ordem inversa com índices progressivos
void mostrar_pilha_verde_recursivo(int index) {

    if (index < 0) {
        return;
    }
    mostrar_pilha_verde_recursivo(index - 1);
    printf("[%d] ", index);
    print_pokemon(stack[index]);
}

//encapsulamento para iniciar o processo
void mostrar_pilha_verde() {
    mostrar_pilha_verde_recursivo(topo);
}
...
void ler_e_executar_comandos(const char* filename) {
    ...
    if (fgets(s, sizeof(s), stdin) != NULL) {
        ...
        // EXECUTAR
        if (strcmp(partes[0], "R") == 0) {

            pokemon* removido = removerPilha();

			if (removido != NULL) {

				printf("(R) %s\n", removido->name);

				//liberamos a memória após usar o pokemon removido
				free_pokemon(removido);
			}
...
int main () {
	...
	while(acabar == 0){
		...
		if (strcmp(linha, "FIM") == 0) {
			...
			if (processed_line) {
                ...
                inserirPilha(parse_poke_line(processed_line));
    ...
    //printf("qual a quantidade de comandos?  ");
    int qtd = 0;
    scanf("%d", &qtd);
    getchar();
    ...
    while (qtd > 0){
        ...
        ler_e_executar_comandos(filename);
	...
	//liberar memória alocada para os pokémons na pilha
    for (int i = 0; i <= topo; i++) {
        free_pokemon(stack[i]);
    }
