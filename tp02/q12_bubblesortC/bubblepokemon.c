...
void bubbleID (pokedex* mypokedex){
    ...
    for (int i = n-1; i > 0; i--){
        ...
        for (int j = 0; j < i; j++){
            ...
            //coloquei o caso de empate aqui mas acho que ele nunca será usado porque cada id é exclusivo de seu respectivo pokemon
            if((mypokedex->arr[j].id == mypokedex->arr[j+1].id) && (strcmp(mypokedex->arr[j].name, mypokedex->arr[j+1].name) > 0)){
                swap_pokedex(mypokedex, j, j+1); 
            }

            if (mypokedex->arr[j].id > mypokedex->arr[j+1].id){
                swap_pokedex(mypokedex, j, j+1);    
            }
