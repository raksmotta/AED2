...
void insercaoPorCor(pokedex* mypokedex, int n, int cor, int h){

    for (int i = (h + cor); i < n; i+=h) {

        pokemon tmp = mypokedex->arr[i];
        int j = i - h;

        //se o peso de j for maior de que tmp
		//OU se os pesos forem iguais e o nome de j for maior que o de tmp
        while ((j >= 0) && 
              (mypokedex->arr[j].weight > tmp.weight || 
              (mypokedex->arr[j].weight == tmp.weight && strcmp(mypokedex->arr[j].name, tmp.name) > 0))) {

            mypokedex->arr[j + h] = mypokedex->arr[j];
            j -= h;
        }

        mypokedex->arr[j + h] = tmp;
...
void shellsort(pokedex* mypokedex, int n){
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++){
            ...
            insercaoPorCor(mypokedex, n, cor, h);
...
int main () {
    ...
    shellsort(mypokedex, mypokedex->size);
