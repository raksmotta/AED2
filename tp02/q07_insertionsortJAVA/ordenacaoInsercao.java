...
public class ordenacaoInsercao{
    ...
    public static Pokedex insertion (Pokedex mypokedex){

        int n = mypokedex.size();

        for (int i = 1; i < n; i++){

            Pokemon current = mypokedex.get(i);

            int j = i - 1;

            //compareTo retorna 0 se as duas datas forem iguais e um inteiro positivo se a data1 vem DEPOIS da data2
            while (j>=0 && mypokedex.get(j).get_captureDate().compareTo(current.get_captureDate())>=0){

                //SE as duas datas forem exatamente iguais, então vamos ao critério de desempate: o NOME. aquele com o maior nome deve ir para frente
                if (mypokedex.get(j).get_captureDate().compareTo(current.get_captureDate())==0){

                    if(mypokedex.get(j).get_name().compareTo(current.get_name()) > 0){

                        mypokedex.set(j+1, mypokedex.get(j));
                        j--;
    ...
    public static void main(String[] args) {
        ...
        Pokedex myinsertionpokedex = insertion(mypokedex);

        for (int x = 0; x < myinsertionpokedex.size(); x++){
            System.out.println(myinsertionpokedex.get(x).pokemonString());
        }
