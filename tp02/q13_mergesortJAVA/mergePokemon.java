...
public class mergePokemon {

    public static void mergeSort (Pokedex mypokedex, int left, int right){

        if(left < right){

            //mesmo que (left + right)/2 mas evita overflow
            int mid = left + (right - left) / 2;

            mergeSort(mypokedex, left, mid);
            mergeSort(mypokedex, mid + 1, right);

            merge(mypokedex, left, mid, right);
        }
    }

    public static void merge (Pokedex mypokedex, int left, int mid, int right){
        ...
        Pokemon[] arrLeft = new Pokemon [sizeLeft];
        Pokemon[] arrRight = new Pokemon [sizeRight];


        for (int i = 0; i < sizeLeft; i++){

            arrLeft[i] = mypokedex.arr[left + i];

        }

        for(int j = 0; j < sizeRight; j++) {

            arrRight[j] = mypokedex.arr[mid+1 + j];
        ...
        while (indexLeft < sizeLeft && indexRight<sizeRight){
            ...
            int typeComparison = arrLeft[indexLeft].get_types().get(0).compareTo(arrRight[indexRight].get_types().get(0));

            if (typeComparison < 0){

                //o tipo da esquerda vem ANTES do tipo da direita
                mypokedex.arr[mergedArrayIndex] = arrLeft[indexLeft];
                indexLeft ++;
                
            }else if (typeComparison > 0){

                //o tipo da DIREITA é o que vem antes, pois o da ESQ é MAIOR
                mypokedex.arr[mergedArrayIndex] = arrRight[indexRight];
                indexRight ++;

            }else{

                //se os tipos forem iguais, desempatamos com o nome
                //a funcao criada retorna -1 se o nome da ESQUERDA for MENOR do que o da direita (left vem ANTES)
                if (compareNamesforMerge(arrLeft[indexLeft].get_name(), arrRight[indexRight].get_name()) < 0){
    ...
    public static int compareNamesforMerge (String first, String second){
    ...
    public static void main(String[] args) {
        ...
        //primeiro uso um algoritmo ESTÁVEL para ordenar por NOME para DEPOIS chamar o merge nos tipos
        mypokedex.insertionName();

        mergeSort(mypokedex, 0, mypokedex.size()-1);
...
class Pokedex {
    ...
    public void insertionName(){

        int n = size();

        for (int i = 1; i < n; i++){

            Pokemon current = arr[i];

            int j = i - 1;

            while (j>=0 && arr[j].get_name().compareTo(current.get_name()) > 0){
