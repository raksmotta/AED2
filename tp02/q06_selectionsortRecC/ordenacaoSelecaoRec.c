...
pokedex* create_pokedex (){
    ...
    mypokedex->arr = (pokemon*)malloc(900 * sizeof(pokemon));
...
int selectionSortRecursivo (pokedex* mypokedex, int n, int index){

	//caso base: se o array inteiro já foi ordenado
	if (index >= n-1){
		return 0;
	}
    
	//achar a posicao do menor elemento da parte ainda nao ordenada do vetorS
	int minIndex = index;
	for (int i = index+1; i<n; i++){

		if(strcmp(mypokedex->arr[i].name, mypokedex->arr[minIndex].name) < 0){
			minIndex = i;
		}
	}

	//fazer o swap entre o menor elemento que acabamos de encontrar com o primeiro elemento não ordenado ainda
	if(minIndex != index){

		pokemon tmp = mypokedex->arr[index];
		mypokedex->arr[index] = mypokedex->arr[minIndex];
		mypokedex->arr[minIndex] = tmp;

	}

	//chamar a funcao recursivamente para o restante da porção ainda não ordenada
	selectionSortRecursivo(mypokedex, n, index+1);
...
int main () {
	...
	selectionSortRecursivo(mypokedex, mypokedex->size, 0);
