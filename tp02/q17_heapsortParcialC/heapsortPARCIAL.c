...
char* rewritePokeDate (char* date){

	//a data esta guardada em cada pokemon em uma string no formato dd/MM/yyyy
	//queremos passar para o formato yyyy-MM-dd para que as datas sejam comparáveis usando strcmp

	//10 caracteres + 1 para o null terminator
	char* formattedDate = malloc(11 * sizeof(char)); 
	...
	//preenchendo os 4 caracteres do ano
	for (int i = 6; i < 10; i++){
		formattedDate[posFormatted] = date[i];
		posFormatted++;
	}
...
//o criterio de ordenacao para o heapsort eh HEIGHT e o desempate eh feito pelo nome
int comparePokemonforHeapsort(pokemon a, pokemon b) {

    //se a altura de a for MAIOR do que de b, a deve vir DEPOIS na construção do heap (ficar mais acima)
    if (a.height > b.height) {
        return 1;
    
    //se a altura de a for MENOR do que de b, a deve vir ANTES na formação heap (ficar mais embaixo)
...
/*void heapify(pokedex* mypokedex, int n, int i) {

    //inicializar o largest como a raiz
    int largest = i;

    //index da left child
    int left = 2 * i + 1;

    //index da right child
    int right = 2 * i + 2;

    //SE a left child existe e é maior que a raiz
    if (left < n && comparePokemonforHeapsort(mypokedex->arr[left], mypokedex->arr[largest]) > 0) {
        largest = left;
    }

    //Se a right child existe e é maior que o largest so far
    if (right < n && comparePokemonforHeapsort(mypokedex->arr[right], mypokedex->arr[largest]) > 0) {
        largest = right;
    }

    //se o largest encontrado não for a raiz, swapamos o i (posição da raiz) com o largest e chamamos a função recursivamente
    if (largest != i) {
        swap_pokedex(mypokedex, i, largest);

        //recursively heapify the affected sub-tree
        heapify(mypokedex, n, largest);
    }
}*/

//heapSize define o tamanho atual do heap
void heapify(pokedex* mypokedex, int heapSize, int i) {
    ...
    if (left < heapSize && comparePokemonforHeapsort(mypokedex->arr[left], mypokedex->arr[largest]) > 0) {
        largest = left;
    }

    //se o filho direito existe e é maior que o maior até agora
    if (right < heapSize && comparePokemonforHeapsort(mypokedex->arr[right], mypokedex->arr[largest]) > 0) {
        largest = right;
    }

    //se o maior encontrado ainda não está a raiz, temos que continuar refazendo o heap
    if (largest != i) {
        swap_pokedex(mypokedex, i, largest);
        heapify(mypokedex, heapSize, largest);
    }
}


void heapSort(pokedex* mypokedex) {

    int n = mypokedex->size;

    //rearrange array and build max-heap (max heap é aquele com o MAIOR elemento na raiz)
    for (int i = n / 2 - 1; i >= 0; i--) {
        heapify(mypokedex, n, i);
    }
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        swap_pokedex(mypokedex, 0, i);

        //call max heapify on the reduced heap
        heapify(mypokedex, i, 0);
    }
}

//ordena apenas os primeiros k elementos do vetor
void partialHeapSort(pokedex* mypokedex, int k) {

    int n = mypokedex->size;

    //passo 1: construir um heap máximo com os primeiros k elementos
    for (int i = k / 2 - 1; i >= 0; i--) {
        heapify(mypokedex, k, i);
    }

    //passo 2: iterar sobre os elementos restantes e manter os k menores no heap
    for (int i = k; i < n; i++) {

        //se o elemento atual for menor que a raiz do heap
        if (comparePokemonforHeapsort(mypokedex->arr[i], mypokedex->arr[0]) < 0) {

            //substituir a raiz pelo novo elemento
            swap_pokedex(mypokedex, 0, i);

            //reorganizar o heap para manter a propriedade de max heap
            heapify(mypokedex, k, 0);
        }
    }

    //passo 3: ordenar os k elementos no heap máximo
    //isso colocará os k menores elementos nas primeiras k posições do vetor em ordem crescente
    for (int i = k - 1; i >= 0; i--) {

        //mover a raiz atual para o final do heap
        swap_pokedex(mypokedex, 0, i);

        //reduzir o tamanho do heap e heapificar a raiz novamente
        heapify(mypokedex, i, 0);
    }
}
