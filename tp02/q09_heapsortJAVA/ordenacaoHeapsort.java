...
class Pokedex {
    ...
    //precisamos dessa função para implementar o critério de desempate do heapsort
    private int comparePokemonforHeapsort(Pokemon a, Pokemon b) {
        ...
        //se a altura de a for MENOR do que de b, a deve vir ANTES na formação heap (ficar mais embaixo)
            ...
            //SE as alturas forem IGUAIS, comparamos os nomes por ordem alfabética
            //retorna um inteiro negativo se a vier antes de b no alfabeto
            //retorna positivo se a vier depois de b no alfabeto
    ...
    private void heapify(int n, int i) {
        ...
        if (left < n && comparePokemonforHeapsort(arr[left], arr[largest]) > 0) {
            largest = left;
        }
    
        //Se a right child existe e é maior que o largest so far
        if (right < n && comparePokemonforHeapsort(arr[right], arr[largest]) > 0) {
            largest = right;
        }
