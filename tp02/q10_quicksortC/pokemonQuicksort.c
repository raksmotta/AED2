...
int comparePokemonforQuicksort(pokemon a, pokemon pivo) {

    if (a.generation < pivo.generation) {
        return -1;

    } else if (a.generation > pivo.generation) {
        return 1;
        ...
        return strcmp(a.name, pivo.name);
    }
}

//quando formos chamar a funcao, esq tem que ser 0 e dir tem que ser tamanho do vetor - 1
void quicksortRec(pokedex* mypokedex, int esq, int dir) {

    int i = esq;
    int j = dir;

    pokemon pivo = mypokedex->arr[(dir+esq)/2];
    ...
    while (i <= j) {
        ...
        while (comparePokemonforQuicksort(mypokedex->arr[i], pivo) < 0) i++;
        ...
        while (comparePokemonforQuicksort(mypokedex->arr[j], pivo) > 0) j--;

        if (i <= j) {
            swap_pokedex(mypokedex, i, j);
            i++;
            j--;
        }
    }

    if (esq < j)  quicksortRec(mypokedex, esq, j);
    if (i < dir)  quicksortRec(mypokedex, i, dir);
...
int main () {
	...
	while(acabar == 0){
		...
		if (strcmp(linha, "FIM") == 0) {
			...
			if (processed_line) {
                ...
                pokemon* new_pokemon = parse_poke_line(processed_line);
    ...
    quicksortRec(mypokedex, 0, mypokedex->size - 1);
