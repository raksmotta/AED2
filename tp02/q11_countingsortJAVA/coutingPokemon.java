...
public class coutingPokemon {
    ...
    public static void main(String[] args) {
        ...
        //primeiro uso um algoritmo ESTÁVEL para ordenar por NOME para DEPOIS chamar o counting no captureRate
        mypokedex.insertionName();

        mypokedex.countingSort();
...
class Pokedex {
    ...
    public void countingSort(){
        ...
        int min = arr[0].get_captureRate();
        int max = arr[0].get_captureRate();

        //vou ter que iterar uma vez sobre o array para achar o maior e o menor valor de capture rate
        for (int i = 1; i < n; i++) {

            if (arr[i].get_captureRate() > max) {

                max = arr[i].get_captureRate();

            } else if (arr[i].get_captureRate() < min) {

                min = arr[i].get_captureRate();

            }
        }

        //o RANGE de valores no array inputado é o seguinte:
        int k = max - min + 1;

        //inicializando array de CONTAGEM (o tamanho dele é o range)
        int[] count = new int[k];

        //contando a quantidade de ocorrências de cada valor
        for (int pos = 0; pos < n; pos++) {

            int value = arr[pos].get_captureRate();
        ...
        //calcular a soma cumulativa de cada posição
        for (int i = 1; i < k; i++) {
        ...
        //inicializando o array que será o final ORDENADO
        Pokemon[] output = new Pokemon[size];

        //colocando os pokemons no array de output na sequência ordenada
        //para manter a estabilidade devemos iterar sobre o vetor de trás para frente
        for (int i = size - 1; i >= 0; i--) {

            int current = arr[i].get_captureRate();
            int index = current - min;

            /*Compute the total number of elements less than or equal to the current value (count[index]), and place the current element at the index just before that total (since indices start at 0). */
