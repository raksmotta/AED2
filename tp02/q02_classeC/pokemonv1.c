...
pokemon* parse_poke_line (const char* line) {
	...
	*start_bracket = '\0';
	...
	for (int x = 0; x < p->abilities_size; x++) {
		printf("habilidade: %s\n", p->abilities[x]);
	}
	...
	if (strcmp(tokens_after[after_index], "yy") != 0 && strcmp(tokens_after[after_index], "") != 0) {
		...
		p->captureDate = tokens_after[after_index];
...
void print_pokemon (pokemon* p){
	...
	printf("] - %dkg - %dm - %d%% - ", p->weight, p->height, p->captureRate);
	...
	printf("%dgen] - %s", p->generation, p->captureDate);
