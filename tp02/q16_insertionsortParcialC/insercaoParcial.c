...
//a chave de pesquisa é o atributo captureDate e o desempate é feito pelo nome
void insertionPARCIAL (pokedex* mypokedex, int k) {
    ...
    for (int i = 1; i < n; i++) {
		...
		/*quando i >= k, o objetivo é inserir o Pokémon atual apenas nos primeiros k elementos já ordenados. usamos j = k - 1 porque a comparação do elemento atual e inserção só precisam acontecer até o limite de k. o Pokémon atual não precisa ser comparado ou inserido além dessa parte já organizada*/
		if (i < k){
        ...
        while ((j >= 0) && (strcmp(rewritePokeDate(mypokedex->arr[j].captureDate), rewritePokeDate(atual.captureDate)) >= 0)) {

			//desempatando pelo nome se as datas forem iguais
			if (strcmp(rewritePokeDate(mypokedex->arr[j].captureDate), rewritePokeDate(atual.captureDate)) == 0){

				if (strcmp(mypokedex->arr[j].name, atual.name) > 0){
