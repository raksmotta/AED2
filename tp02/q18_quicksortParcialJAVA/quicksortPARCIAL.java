...
public class quicksortPARCIAL{
    ...
    public static void main(String[] args) {
        ...
        mypokedex.quicksortRecParcial(0, mypokedex.size() - 1);
...
class Pokedex {
    ...
    public void swap (int pos1, int pos2){
        ...
        Pokemon tmp = arr[pos1];
        arr[pos1] = arr[pos2];
        arr[pos2] = tmp;

    }

    //essa função é necessária para podermos implementar o caso de EMPATE entre generations iguais através da comparação do nome
    //passamos o ENDEREÇO dos pokemons no array
    public int comparePokemonforQuicksort(Pokemon a, Pokemon pivo) {

        if (a.get_generation() < pivo.get_generation()) {

            return -1;

        } else if (a.get_generation() > pivo.get_generation()) {

            return 1;

        } else {

            //SE as generations forem iguais, comparar os nomes em ordem alfabética 
            //o strcmp retorna um valor negativo se a primeira string for lexicograficamente menor (ou seja, vem antes em ordem alfabética)
            //retorna um numero positivo se a string1 for maior que a string2
            return a.get_name().compareTo(pivo.get_name());
        }
    }

    //quando formos chamar a funcao, esq tem que ser 0 e dir tem que ser tamanho do vetor - 1
    //para fazer parcial, basta abandonar a partição à direita quando a da esquerda tiver k ou mais itens
    public void quicksortRecParcial(int esq, int dir) {

        int i = esq;
        int j = dir;

        Pokemon pivo = arr[(dir+esq)/2];

        while (i <= j) {

            //se a generation do i for menor que a generation do pivo OU
            //as generations forem iguais e o nome do i for menor do que o do pivo
            //a funcao vai retornar -1 e aí continua dentro do while
            while (comparePokemonforQuicksort(arr[i], pivo) < 0) i++;

            //se a generation do j for maior que a generation do pivo OU
            //as generations forem iguais e o nome do j for maior do que o do pivo
            //a funcao vai retornar 1 e o loop do while prossegue
            while (comparePokemonforQuicksort(arr[j], pivo) > 0) j--;
        ...
        if (esq < j)  quicksortRecParcial(esq, j);
        if (i < k && i < dir)  quicksortRecParcial(i, dir);
